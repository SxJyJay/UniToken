...
class FinetuneSolverBase(ABC):
    ...
    def train_one_epoch(
        ...
        # # Test torch.profiler functions
        # from torch.profiler import profile, record_function, ProfilerActivity
        # def trace_handler(p):
        #     output = p.key_averages().table(sort_by="self_cuda_time_total", row_limit=10)
        #     print(output)
        #     p.export_chrome_trace("/mnt/dolphinfs/hdd_pool/docker/user/hadoop-basecv/jiaoyang/jiaoyang/Lumina-mGPT/debug_outs/tmp/trace_" + str(p.step_num) + ".json")
